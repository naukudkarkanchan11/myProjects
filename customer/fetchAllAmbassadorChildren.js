...
router.post('/', function (req, res) {
    trycatch(function (err, data) {
           setTimeout(function () {
                ...
                var customer_id1 = req.body.customer_id;//ambessador_id
                ...
                if (customer_id == undefined) {
                    ...
                    customers.find({"customer_id": customer_id,"isAmbassador":true}, function(err, docs){
                              if(err){
                                  ...
                                  console.log("DL"+docs.length);
                                  if(docs.length !== 0){
                                  docs.forEach(function(doc1){
                                    console.log("Cid"+doc1.customer_id);           
                   
                                var x;
                                fetch_children(doc1.customer_id,resultCallback1);
...
function fetch_children(customer_id,callback){ 
          ...
          console.log("C"+customer_id);
          customers.find({"referral_id": customer_id},'customer_id email referral_id payback joiningDate lastUpdated',
                                function(err, doc1){
              if(err){
                  console.log("Error"+err);
              }else{
